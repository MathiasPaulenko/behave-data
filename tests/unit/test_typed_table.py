...
class TestCleanHeaders:
    def test_plain_headers(self, make_table: object) -> None:
        table = make_table(["name", "age"], [["Alice", "30"]])
        tw = TypedTableWrapper(table)
        result = tw.clean_headers()
        assert result == ["name", "age"]

    def test_typed_headers(self, make_table: object) -> None:
        table = make_table(["name:str", "age:int"], [["Alice", "30"]])
        tw = TypedTableWrapper(table)
        result = tw.clean_headers()
        assert result == ["name", "age"]

    def test_mixed_typed_and_untyped(self, make_table: object) -> None:
...
class TestTypedDicts:
    def test_int_conversion(self, make_table: object) -> None:
        table = make_table(["age:int"], [["30"], ["25"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"age": 30}, {"age": 25}]

    def test_float_conversion(self, make_table: object) -> None:
        table = make_table(["price:float"], [["3.14"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"price": 3.14}]

    def test_bool_conversion(self, make_table: object) -> None:
        table = make_table(["active:bool"], [["true"], ["false"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"active": True}, {"active": False}]

    def test_date_conversion(self, make_table: object) -> None:
        table = make_table(["born:date"], [["2024-01-15"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"born": date(2024, 1, 15)}]

    def test_str_conversion(self, make_table: object) -> None:
        table = make_table(["name:str"], [["Alice"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"name": "Alice"}]

    def test_no_type_returns_string(self, make_table: object) -> None:
        table = make_table(["name"], [["Alice"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"name": "Alice"}]

    def test_null_markers_empty_cell_to_none(self, make_table: object) -> None:
        table = make_table(["name"], [[""]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"name": None}]

    def test_null_marker_null_to_none(self, make_table: object) -> None:
        table = make_table(["age:int"], [["null"], ["30"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"age": None}, {"age": 30}]

    def test_nullable_type_with_empty_cell(self, make_table: object) -> None:
        table = make_table(["age:int?"], [[""], ["30"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"age": None}, {"age": 30}]

    def test_non_nullable_type_empty_cell_still_none(self, make_table: object) -> None:
        table = make_table(["age:int"], [[""]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"age": None}]

    def test_per_column_markers(self, make_table: object) -> None:
    ...
    def test_without_config_uses_defaults(self, make_table: object) -> None:
        table = make_table(["name"], [["null"]])
        tw = TypedTableWrapper(table)
        assert tw.typed_dicts() == [{"name": None}]

    def test_config_override_in_call(self, make_table: object) -> None:
        table = make_table(["name"], [["<empty>"]])
        tw = TypedTableWrapper(table)
        cfg = Config.from_dict({"null_markers": ["<empty>"]})
        assert tw.typed_dicts(cfg) == [{"name": None}]

    def test_uses_row_get_header_not_positional(self, make_table: object) -> None:
        table = make_table(["name:str", "age:int"], [["Alice", "30"]])
        tw = TypedTableWrapper(table)
        result = tw.typed_dicts()
        assert result[0] == {"name": "Alice", "age": 30}

    def test_invalid_type_raises(self, make_table: object) -> None:
    ...
    def test_one_row_one_column(self, make_table: object) -> None:
...
class TestTypedObjects:
    def test_dataclass_conversion(self, make_table: object) -> None:
        @dataclass
        class User:
            name: str
            age: int
    ...
    def test_pydantic_model(self, make_table: object) -> None:
...
class TestToDict:
    def test_two_columns_key_value(self, make_table: object) -> None:
        table = make_table(["key", "value"], [["a", "1"], ["b", "2"]])
        tw = TypedTableWrapper(table)
        result = tw.to_dict()
        assert result == {"a": "1", "b": "2"}

    def test_one_column_raises(self, make_table: object) -> None:
        table = make_table(["name"], [["Alice"]])
        tw = TypedTableWrapper(table)
        with pytest.raises(ValueError, match="2 columns"):
            tw.to_dict()

    def test_three_columns_raises(self, make_table: object) -> None:
        table = make_table(["a", "b", "c"], [["1", "2", "3"]])
        tw = TypedTableWrapper(table)
        with pytest.raises(ValueError, match="2 columns"):
            tw.to_dict()

    def test_typed_columns(self, make_table: object) -> None:
...
class TestToLists:
    def test_returns_list_of_lists(self, make_table: object) -> None:
...
class TestToPandas:
    def test_returns_dataframe(self, make_table: object) -> None:
    ...
    def test_missing_pandas_raises(self, make_table: object) -> None:
        table = make_table(["name:str"], [["Alice"]])
        tw = TypedTableWrapper(table)
...
class TestInheritance:
    def test_as_dicts_still_works(self, make_table: object) -> None:
        table = make_table(["name", "age"], [["Alice", "30"]])
        tw = TypedTableWrapper(table)
        assert tw.as_dicts() == [{"name": "Alice", "age": "30"}]

    def test_transpose_still_works(self, make_table: object) -> None:
        table = make_table(["name", "age"], [["Alice", "30"]])
        tw = TypedTableWrapper(table)
        transposed = tw.transpose()
        assert transposed is not None

    def test_find_row_still_works(self, make_table: object) -> None:
        table = make_table(["name", "age"], [["Alice", "30"], ["Bob", "25"]])
        tw = TypedTableWrapper(table)
        row = tw.find_row(name="Alice")
        assert row is not None

    def test_headers_still_works(self, make_table: object) -> None:
        table = make_table(["name:str", "age:int"], [])
        tw = TypedTableWrapper(table)
        assert tw.headers == ["name:str", "age:int"]

    def test_len_still_works(self, make_table: object) -> None:
...
class TestTypedWrap:
    def test_returns_typed_table_wrapper(self, make_table: object) -> None:
        table = make_table(["name:str"], [["Alice"]])
        tw = typed_wrap(table)
        assert isinstance(tw, TypedTableWrapper)
        assert tw.typed_dicts() == [{"name": "Alice"}]

    def test_with_config(self, make_table: object) -> None:
        table = make_table(["name"], [["<empty>"]])
        cfg = Config.from_dict({"null_markers": ["<empty>"]})
